...
public class VideoView2 extends TextureView
    ...
    public VideoView2(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            //todo implement AudioAttributes
        }
        setSurfaceTextureListener(new SurfaceTextureListener() {
            @Override
            public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
                mSHCallback.surfaceCreated(new SurfaceHolderImpl(surface));
            }

            @Override
            public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) {
                mSHCallback.surfaceChanged(null, 0, width, height);
            }
    ...
    public void addSubtitleSource(InputStream is, MediaFormat format) {
        if (mMediaPlayer == null) {
            ...
            try {
// todo               mMediaPlayer.addSubtitleSource(is, format);
...
//            mAudioManager.requestAudioFocus(null, mAudioAttributes, mAudioFocusType, 0 /*flags*/);
        ...
        try {
            ...
            for (Pair<InputStream, MediaFormat> pending : mPendingSubtitleTracks) {
                try {
// todo                   mMediaPlayer.addSubtitleSource(pending.first, pending.second);
                } catch (IllegalStateException e) {
                    mInfoListener.onInfo(
                            mMediaPlayer, MediaPlayer.MEDIA_INFO_UNSUPPORTED_SUBTITLE, 0);
                }
    ...
    MediaPlayer.OnPreparedListener mPreparedListener = new MediaPlayer.OnPreparedListener() {
        public void onPrepared(MediaPlayer mp) {
            ...
            Metadata data = Metadata.getMetadata(mp);
            if (data != null) {
    ...
    private void setFixedSize(int width, int height) {
        ...
        if (width > 0 && height > 0 && getWidth() > 0 && getHeight() > 0) {
            ...
            float aspectRatio2 = 1f * getWidth() / getHeight();
            if (aspectRatio > aspectRatio2) {
                setScaleX(1);
                setScaleY(getWidth() / aspectRatio / getHeight());
            } else if (aspectRatio < aspectRatio2) {
                setScaleX(getHeight() * aspectRatio / getWidth());
                setScaleY(1);
            } else {
                setScaleX(1);
                setScaleY(1);
    ...
    public Bitmap getBitmap() {
        return super.getBitmap((int) (getWidth() * getScaleX()), (int) (getHeight() * getScaleY()));
    }

    @interface UnsupportedAppUsage {
    ...
    static class Metadata {
        ...
        public static Metadata getMetadata(MediaPlayer mediaPlayer) {
            Metadata metadata = null;
            try {
                metadata = new Metadata();
                metadata.mMediaMetadataRetriever.setDataSource("");
    ...
    static class SurfaceHolderImpl implements android.view.SurfaceHolder {
        private Surface mSurface;

        public SurfaceHolderImpl(SurfaceTexture surfaceTexture) {
